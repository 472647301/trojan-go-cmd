...
async function main() {
  ...
  await execSync('rm -rf /etc/trojan-go')
  ...
  if (bt) {
    ...
    await execSync('systemctl disable nginx')
    await execSync(`${pmt} remove -y nginx`)
    if (pmt === 'apt') {
      await execSync(`${pmt} remove -y nginx-common`)
    }
    await execSync('rm -rf /etc/nginx/nginx.conf')
    if (existsSync('/etc/nginx/nginx.conf.bak')) {
      await execSync('mv /etc/nginx/nginx.conf.bak /etc/nginx/nginx.conf')
    }
    await execSync(`rm -rf ${trojanPath.nginxConf}${domain}.conf`)
  }
  await execSync('~/.acme.sh/acme.sh --uninstall')
